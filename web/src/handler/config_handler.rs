...
pub async fn get_config_handler(config: web::Data<Arc<RwLock<Config>>>) -> impl Responder {
    let config = config.read().unwrap().clone().convert();
    JsonResponse(HttpResult::<ConfigVo>::success(Some(config)))
}

pub async fn get_server_config_handler(config: web::Data<Arc<RwLock<Config>>>) -> impl Responder {
    let config = config.read().unwrap().clone().convert();
    JsonResponse(HttpResult::<ServerConfigVo>::success(Some(config.server)))
}

pub async fn set_server_config_handler(
...
pub async fn get_app_config_handler(config: web::Data<Arc<RwLock<Config>>>) -> impl Responder {
    let config = config.read().unwrap().clone().convert();
    JsonResponse(HttpResult::<AppConfigVo>::success(Some(config.app)))
}

pub async fn set_app_config_handler(
    config: web::Data<Arc<RwLock<Config>>>,
...
pub async fn get_web_server_config_handler(
    config: web::Data<Arc<RwLock<Config>>>,
) -> impl Responder {
    let config = config.read().unwrap().clone().convert();
...
pub async fn set_web_server_config_handler(
    config: web::Data<Arc<RwLock<Config>>>,
