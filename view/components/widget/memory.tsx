    ...
    <Card className="relative ">
      ...
      <CardContent>
        ...
        <p className="text-xs text-muted-foreground">
          {memoryUsage?.total ? formatToString(memoryUsage?.total) : "N/A"} of
          total
        </p>
      </CardContent>
      <ResponsiveContainer className="absolute top-[50%] right-0 transform translate-y-[-50%]">
        <PieChart>
          <Pie
            dataKey="value"
            data={chartData}
          ...
          >
            {chartData.map((data, index) => (
              <Cell
                key={data.key}
                className="outline-none cursor-pointer stroke-none"
