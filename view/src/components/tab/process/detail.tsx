  ...
  return process ? (
    <ScrollArea className="h-[calc(100vh-65px-1rem-36px-40px-16px-20px)] w-full border rounded-lg">
      <Card className="h-full border-none shadow-none">
        <CardHeader className="mb-6 px-6 py-1 rounded-t-lg sticky top-0 z-[100] flex flex-row justify-between items-center space-x-2 space-y-0 bg-muted">
              ...
              <Col numColSpan={3}>
                <TremorCard>
                  ...
                  <div className="flex justify-between">
                    {process?.disk &&
                      ["read", "write", "total_read", "total_write"].map(
                        (key, index) => {
                          return (
                            <div key={`${key}-${index}`}>
                              <Text>{key}</Text>
                              <Bold>
                                {formatToString(
                                  process?.disk[key as keyof DiskIO]
                                )}
                              </Bold>
                            </div>
                          )
                ...
                <TremorCard>
                  ...
                  <ScrollArea className="h-64">
                    {process?.environ.map((env, index) => {
                      return (
                        <STooltip key={`${env}-${index}`} content={env}>
